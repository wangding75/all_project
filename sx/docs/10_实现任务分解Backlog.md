...
> 状态初始化规则为 `planned`（blocked on Phase 0–4）；当前实际进度以本表为
> 准，其中 P-A 已完成，P-B～P-F 仍为 `planned`。

| Epic | 名称 | 状态 | 说明 |
|------|------|------|------|
| P-A | 商业授权（服务端+客户端） | done | 四阶段后 |
...
## Epic P-A — 商业授权（客户端+服务端）

| ID | 任务 | 状态 |
|----|------|------|
| P-A01 | 授权方案设计文档 | done |
| P-A02 | 服务端：发卡/核销/设备绑定/解绑 | done |
| P-A03 | 客户端：激活改走服务端 + 本地 Token | done |
| P-A04 | Android 14/15/16 兼容性修复 | done |
| P-A05 | DEV 卡密仅 debug 包保留 | done |
| P-A06 | cleartext HTTP 限 debug 专用 | done |
...
| ID | 任务 | 状态 | 规模 | 依赖 | 验收要点 |
|----|------|------|------|------|----------|
| F-01 | ≥ 2 机型回归 | **done** | M | E | **ST-1 MUMU 模拟器 (Android 12) PASS @ 0777c15；真机回归当 ST-2 另场进行** |
| F-02 | 协议/隐私页 | done | S | B-03 | 可打开 |
| F-03 | R8 与 keep 规则 | done | M | C | release 可装 |
| F-04 | 使用说明与已知问题 | done | S | — | README 更新 |
| F-05 | v1.0 RC 清单勾选 | done | S | F-01 | PRD MVP 全过 |
