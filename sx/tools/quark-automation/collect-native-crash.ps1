...
function Invoke-AdbShell {
    ...
    # Write-Host "[DEBUG] Shell command: $normalizedCommand" -ForegroundColor Yellow
...
function Get-PackageUid {
    ...
    $pmText = if ($pmRes.ExitCode -eq 0) { $pmRes.Output } else { "" }
    if ($OutputDir -and (Test-Path -LiteralPath $OutputDir -PathType Container)) {
        Write-Utf8Text -Path (Join-Path $OutputDir "package-uid-pm-$safePkg.txt") -Content $pmText
    }

    $pmUid = $null
    if ($pmText) {
        $escapedPkg = [regex]::Escape($PackageName)
        $pmMatch = [regex]::Match($pmText, "(?m)^package:${escapedPkg}\s+uid:(?<uid>\d+)\s*$")
        if ($pmMatch.Success) {
            $pmUid = [int]$pmMatch.Groups['uid'].Value
        }
    }

    $dumpRes = Invoke-Adb -Arguments @("-s", $DeviceSerial, "shell", "dumpsys", "package", $PackageName) -AllowNonZero
    $dumpText = if ($dumpRes.ExitCode -eq 0) { $dumpRes.Output } else { "" }
    if ($OutputDir -and (Test-Path -LiteralPath $OutputDir -PathType Container)) {
        Write-Utf8Text -Path (Join-Path $OutputDir "package-uid-dumpsys-$safePkg.txt") -Content $dumpText
    }

    $dumpsysUid = $null
    if ($dumpText) {
        $dumpMatch = [regex]::Match($dumpText, "(?m)^\s*userId=(?<uid>\d+)\s*$")
        if ($dumpMatch.Success) {
            $dumpsysUid = [int]$dumpMatch.Groups['uid'].Value
        }
    }

    if (($null -ne $pmUid) -and ($null -ne $dumpsysUid) -and ($pmUid -ne $dumpsysUid)) {
        Write-Host "[!] Package UID mismatch between pm ($pmUid) and dumpsys ($dumpsysUid) for $PackageName" -ForegroundColor Red
        return $null
    }

    if ($null -ne $pmUid) { return $pmUid }
    if ($null -ne $dumpsysUid) { return $dumpsysUid }
    return $null
}
...
function Get-ProcInfo {
    ...
    $command = "if [ -r /proc/$ProcessId/stat ] && [ -r /proc/$ProcessId/status ] && [ -r /proc/$ProcessId/cmdline ]; then stat_line=`$(cat /proc/$ProcessId/stat 2>/dev/null); uid=`$(awk '/^Uid:/{print `$2; exit}' /proc/$ProcessId/status 2>/dev/null); cmd=`$(tr '\000' ' ' < /proc/$ProcessId/cmdline 2>/dev/null | sed 's/[[:space:]]*`$//'); ppid=`$(echo `$stat_line | awk '{print `$4}'); start=`$(echo `$stat_line | awk '{print `$22}'); echo $ProcessId'|'`$ppid'|'`$uid'|'`$start'|'`$cmd; fi"
...
function Get-ExactProcessesByCmdline {
    ...
    $command = "for p in /proc/[0-9]*; do [ -r `$p/cmdline ] || continue; cmd=`$(tr '\000' ' ' < `$p/cmdline 2>/dev/null | sed 's/[[:space:]]*`$//'); case `$cmd in '$escaped') ;; *) continue ;; esac; pid=`${p#/proc/}; stat_line=`$(cat `$p/stat 2>/dev/null); uid=`$(awk '/^Uid:/{print `$2; exit}' `$p/status 2>/dev/null); ppid=`$(echo `$stat_line | awk '{print `$4}'); start=`$(echo `$stat_line | awk '{print `$22}'); echo `$pid'|'`$ppid'|'`$uid'|'`$start'|'`$cmd; done"
...
function Get-TombstoneInventory {
    $command = "for f in /data/tombstones/tombstone_*; do [ -e `$f ] || continue; stat -c '%n|%i|%Y|%s' `$f 2>/dev/null; done"
    $result = Invoke-AdbShell -Command $command -AllowNonZero
    $items = @()
    foreach ($line in ($result.Output -split "\r?\n")) {
        if ([string]::IsNullOrWhiteSpace($line)) { continue }
...
try {
    ...
    $hostUid = Get-PackageUid -PackageName $HostPackage -OutputDir $outputDir
    $targetSystemUid = Get-PackageUid -PackageName $TargetPackage -OutputDir $outputDir
