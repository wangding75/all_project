package com.loc.va.ui.activity;

import arm.Loader;
import com.loc.va.ui.widget.dialog.c;

/* JADX WARN: Classes with same name are omitted:
  D:\github\xh\blackdex_out\classes09.dex
 */
/* compiled from: fuck */
/* loaded from: D:\github\xh\blackdex_out\classes10.dex */
public final /* synthetic */ class u implements c.e {
    private static int[] zL;

    /* renamed from: a, reason: collision with root package name */
    public final /* synthetic */ HomeFragment f22965a;

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ int f22966b;

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ String f22967c;

    static {
        Loader.registerNativesForClass(136);
        native_special_clinit0();
    }

    public /* synthetic */ u(HomeFragment homeFragment, int i5, String str) {
        this.f22965a = homeFragment;
        this.f22966b = i5;
        this.f22967c = str;
    }

    private static native /* synthetic */ void native_special_clinit0();

    @Override // com.loc.va.ui.widget.dialog.c.e
    public final native void a();
}
