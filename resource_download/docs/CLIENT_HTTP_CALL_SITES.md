...
| `client/ui/app.js:apiFetch` | 六个 License-Protected endpoint | 发现受保护 path 后只调用 `pywebview.api.api_request`；没有 bridge 时 `DESKTOP_DEVICE_IDENTITY_REQUIRED` |
...
| `client/desktop/main.py:WindowApi.download_file` | `GET /v1/files/{file_id}` 本地文件交付 | 只使用现有用户凭证；files 当前不在 License Guard scope |
| `client/desktop/http_client.py:is_protected_endpoint` | 当前 Guard 清单 | 仅 jobs create/batch/retry 与 automation PUT/scan；不扩大到 login/search/detail/files/health |
| `client/desktop/device_identity.py` | identity 生命周期 | Windows 用户级 DPAPI；不在项目目录或浏览器存储中保存 private key |
| `client/desktop/device_proof.py` | LS-DEVICE-V3 | 调用 rc3 wheel 官方 helper 生成 activation/request proof、timestamp、nonce、raw body hash |
