...
| 红果下载 | **基本完成** | 搜索/签名/下载/解密链路在模拟器上跑通过 |
| 番茄 Web | 可用但不作主路径 | `web_ssr.py` FONT_MAP；5×10 章采样全成功 |
| 番茄 App 拉密文 | **可用** | 红果签名调 `reader/full` |
| 番茄 App 解密 | **原理与链路已验证** | 进程内 `CryptManager.decrypt` → gzip → HTML |
| 番茄整书产品流水线 | **未完成** | 未稳定接入 `FanqiePlatform.download(mode=app)` |
...
| P0 | **番茄 App 下载流水线未接入** | 不能一键下全书（产品功能缺口） | 短期 |
